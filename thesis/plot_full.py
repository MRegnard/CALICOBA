...
for param_name in PARAM_NAMES:
    ...
    with open(os.path.join(dirname, param_name + '.csv')) as f:
        for cycle, value, action, _ in map(lambda line: line.strip().split(','), f.readlines()[1:]):
            cycles.append(int(cycle))
...
p1, p2, *_ = pathlib.Path(dirname).name.split(';')
p1 = float(p1.split('=')[1])
p2 = float(p2.split('=')[1])
...
for obj_name, obj_d in obj_data.items():
    axes[p].plot(obj_d['cycles'], obj_d['abs_criticalities'], marker='None', linestyle=line_style,
                 label=f'${obj_name}$')
